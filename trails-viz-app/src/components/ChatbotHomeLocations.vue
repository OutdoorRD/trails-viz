<template>
  <div id="chatbotChart" ref="chatbotChart"></div>
</template>
  ...
  methods: {
    ...
    _mergeTrees: function(x, y) {
      ...
      for (let country of y["countries"]) {
        let mergedTreeCountry = findCountry(
          mergedTree["countries"],
          country["name"]
        );
        ...
        for (let state of country["states"]) {
          let mergedTreeState = findState(
            mergedTreeCountry["states"],
            state["name"]
          );
          ...
          for (let county of state["counties"]) {
            let mergedTreeCounty = findCounty(
              mergedTreeState["counties"],
              county["name"]
            );
    ...
    renderTreeMap: function() {
      ...
      if (self.$store.getters.getVizMode === VIZ_MODES.PROJECT) {
        ...
        homeLocationsUrl =
          self.$apiEndpoint + "/sites/" + self.siteid + "/chatbotHomeLocations";
      }

      axios.get(homeLocationsUrl).then((response) => {
    ...
    _renderTreeMap: function() {
      ...
      for (let country of data["countries"]) {
        ...
        for (let state of country["states"]) {
          let stateName = this._checkLabelAndAppend(
            labels,
            state["name"],
            country["name"]
          );
          ...
          for (let county of state["counties"]) {
            let countyName = this._checkLabelAndAppend(
              labels,
              county["name"],
              state["name"]
            );
